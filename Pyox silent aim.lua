--original max fov 360
if not syn or not protectgui then
    getgenv().protectgui = function()end
end
local Library = loadstring(game:HttpGet('https://raw.githubusercontent.com/XPIREDPIRED5/UI-Libs/main/Library.txt'))()
...
local ValidTargetParts = {"Head", "UpperTorso", "LowerTorso", "HumanoidRootPart", "RightFoot", "LeftFoot"};
...
do
    ...
    Main:AddDropdown("TargetPart", {Text = "Target Part", Default = 1, Values = {
        "Head", "UpperTorso", "LowerTorso", "HumanoidRootPart", "RightFoot", "LeftFoot", "Random"
    }})
...
do
    ...
    Main:AddSlider("Radius", {Text = "Radius", Min = 0, Max = 2000, Default = 180, Rounding = 0}):OnChanged(function()
...
resume(create(function()
    RenderStepped:Connect(function()
        if Toggles.MousePosition.Value then 
            if Toggles.aim_Enabled.Value == true and Options.Method.Value == "BLOXFRUIT" then
...
oldIndex = hookmetamethod(game, "__index", function(self, Index)
    if self == Mouse and (Index == "Hit" or Index == "Target") then 
        if Toggles.aim_Enabled.Value == true and Options.Method.Value == "BLOXFRUIT" and getClosestPlayer() then
